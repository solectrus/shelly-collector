...
describe ShellyGen3Adapter do
  subject(:adapter) do
    described_class.new(config:)
  end

  let(:config) { Config.from_env(shelly_host:, shelly_gen: 3, shelly_interval: 5) }
  ...
  describe '#initialize' do
    ...
    it { expect(logger.info_messages).to include('Pulling from your Shelly (Gen3) at http://192.168.178.83/rpc/Shelly.GetStatus every 5 seconds') }
  end
  ...
  describe '#solectrus_record', vcr: 'shelly-pm-mini-gen3' do
